...
- **Skins visuales** (_Retro_, _Neon_, _Pastel_, _Pixel art_): un selector cambia paleta, fondo
  del tablero, rejilla y estilo de dibujo de cada bloque en caliente, sin recargar. La
  preferencia se guarda en `localStorage` bajo la clave `tetris-skin`.
...
- **Skins visuales** (`SKINS`, `applySkin`, `initSkin`): cada entrada de `SKINS` define `colors` (paleta por índice de pieza), `boardBg` (fondo del canvas, o `null` para `clearRect` + fondo del CSS), `grid` (color de rejilla, o `null` para usar el del tema) y `draw`, la función que pinta un bloque — `drawRetro` (plano + highlight, comportamiento previo), `drawNeon` (`shadowBlur` con el color del bloque sobre fondo negro), `drawPastel` (esquinas redondeadas vía `roundRect` con _fallback_ a `arcTo`) y `drawPixel` (textura de _texels_ 5×5 con `shade()` para aclarar/oscurecer). `drawBlock` es ahora un despachador que resuelve el color y delega en `skin.draw`. `applySkin` intercambia la clase `body.skin-*`, repinta al vuelo si hay tablero y guarda la clave `tetris-skin`; se cambia desde el `<select>` del encabezado sin recargar.
